...
const TAG: &str = "5.10.6";

/// Represents a CrateDB docker instance.
/// It is based on the official [`crate` docker image](https://hub.docker.com/_/crate)
///
...
///
/// # Example of postgres wire protocol.
/// ```rust
/// use testcontainers_modules::{cratedb, testcontainers::runners::SyncRunner};
///
...
impl CrateDB {
    ...
    pub fn with_heap_size(mut self, ram_gb: usize) -> Self {
        self.env_vars
            .insert("CRATE_HEAP_SIZE".to_string(), format!("{ram_gb}g"));
        self
    }
}
